...
const C = {
  ...
  greenAccent: "#2E7D55", black: "#000000", cream: "#F7F5F0", creamDark: "#EDE9E1",
  white: "#FFFFFF", red: "#C0392B", redPale: "#FBEAE8",
...
function ProgressBar({ step, total = 4 }) {
  const labels = ["Mortgage", "Debts", "Details", "Results"];
  const pct = (step / (total - 1)) * 100;
  return (
    <div style={{ padding: "0 0 28px 0" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
        {labels.map((l, i) => (
          <span key={l} style={{ fontSize: 11, fontFamily: FONT, fontWeight: i <= step ? 700 : 400, color: i <= step ? C.green : C.gray400, letterSpacing: "0.06em", textTransform: "uppercase", transition: "all 0.4s ease" }}>{l}</span>
        ))}
      </div>
      <div style={{ height: 3, borderRadius: 3, background: C.gray100, overflow: "hidden" }}>
        <div style={{ height: "100%", width: `${pct}%`, background: C.green, borderRadius: 3, transition: "width 0.6s cubic-bezier(0.34,1.56,0.64,1)" }} />
      </div>
    </div>
  );
}
...
function Btn({ children, onClick, variant = "primary", disabled = false, style: extra = {} }) {
  const base = { padding: "14px 28px", borderRadius: 8, fontSize: 15, fontWeight: 700, fontFamily: FONT, cursor: disabled ? "not-allowed" : "pointer", border: "none", transition: "all 0.2s ease", letterSpacing: "0.01em", opacity: disabled ? 0.45 : 1 };
  const styles = {
...
function DebtCard({ debt, index, onUpdate, onRemove, canRemove }) {
  const typeOptions = [
    ...
    { value: "car_loan", label: "Car Loan" }, { value: "bnpl", label: "Buy Now Pay Later" }, { value: "other", label: "Other" },
  ];
      ...
      {[
        ...
        <div key={bar.label} style={{ marginBottom: 14 }}>
          ...
          <div style={{ height: 24, background: C.gray100, borderRadius: 6, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${bar.pct}%`, background: bar.color, borderRadius: 6, transition: "width 1.2s cubic-bezier(0.34,1.56,0.64,1)", transitionDelay: bar.delay }} />
          </div>
        </div>
      ))}
    </div>
...
export default function DebtFlipCalculator() {
  ...
  const s2ok = firstName.trim().length >= 2 && validEmail;

  // Calculations (available from step 1 onward for the headline tease)
  ...
  const flipDebtFreeDate = debtFreeDate(flipYrs);
  ...
  const submitToGHL = async () => {
    setSubmitting(true);
    const debtFreeISO = (() => { const d = new Date(); d.setMonth(d.getMonth() + Math.round(flipYrs * 12)); return d.toISOString().split("T")[0]; })();
    try {
      const params = new URLSearchParams({
        ...
        email: email,
        phone: phone || "",
        // Formatted (for SMS/email merge fields)
        ...
        debtfree_date: flipDebtFreeDate,
        // Raw numerics (for downstream calculations)
        ...
        phone_provided: phone.trim() ? "true" : "false",
  ...
  const headerText = step === 3 ? `${firstName}, here's your Debt Flip`
    : step === 2 ? "You could be debt-free sooner than you think"
    ...
    <>
      ...
      <div ref={containerRef} style={{ minHeight: "100vh", background: C.cream, fontFamily: FONT, overflowY: "auto" }}>
        ...
        <div style={{ background: C.green, padding: "28px 20px 44px", textAlign: "center", position: "relative" }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 800, color: C.white, fontFamily: FONT, lineHeight: 1.2, margin: "0 0 10px 0", maxWidth: 380, marginLeft: "auto", marginRight: "auto" }}>
            {headerText}
          </h1>
          {step < 2 && (
        ...
        <div style={{ maxWidth: 480, margin: "-24px auto 0", padding: "0 16px 40px", position: "relative" }}>
          <div style={{ background: C.cream, borderRadius: 14, padding: step === 3 ? "24px 20px" : "28px 24px", boxShadow: "0 4px 24px rgba(0,0,0,0.06)", border: `1px solid ${C.creamDark}` }}>
            ...
            {step === 1 && (
              <div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                  ...
                  <Btn onClick={() => setStep(2)} disabled={!s1ok}>See My Results →</Btn>
                </div>
              </div>
            )}

            {/* ─── Step 2: Headline Tease + Softened Gate ─── */}
            {step === 2 && (
              <div>
                {/* Ungated headline result */}
                <div style={{ background: C.green, borderRadius: 12, padding: 28, textAlign: "center", marginBottom: 24 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: C.greenPale, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8, fontFamily: FONT }}>
                    Your Debt Flip projection
                  </div>
                  <div style={{ fontSize: 42, fontWeight: 800, color: C.white, fontFamily: FONT, lineHeight: 1.1 }}>
                    {yrsSaved > 0 ? fmtYears(yrsSaved) : "—"}
                  </div>
                  <div style={{ fontSize: 16, fontWeight: 600, color: C.greenPale, fontFamily: FONT, marginTop: 4 }}>
                    sooner you could be debt-free
                  </div>
                  <div style={{ fontSize: 13, color: "#ffffffaa", fontFamily: FONT, marginTop: 12, fontStyle: "italic" }}>
                    Without paying a single dollar more than you already are
                  </div>
                </div>

                {/* Blurred tease of full results */}
                <div style={{ position: "relative", marginBottom: 24 }}>
                  <div style={{ filter: "blur(6px)", opacity: 0.5, pointerEvents: "none", userSelect: "none" }}>
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 10 }}>
                      <StatBox label="Interest Saved" value={fmt(Math.round(intSaved))} large />
                      <StatBox label="Debt-Free By" value={flipDebtFreeDate} large />
                    </div>
                    <ComparisonBars currentYears={mTerm} flipYears={flipYrs} maxYears={mTerm} />
                  </div>
                  <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ background: C.white, borderRadius: 10, padding: "14px 20px", boxShadow: "0 2px 12px rgba(0,0,0,0.1)", textAlign: "center" }}>
                      <span style={{ fontSize: 14, fontWeight: 700, color: C.green, fontFamily: FONT }}>Enter your details below to unlock the full breakdown</span>
                    </div>
                  </div>
                </div>

                {/* Contact form — softened gate */}
                <h2 style={{ fontSize: 19, fontWeight: 800, color: C.black, marginBottom: 4, fontFamily: FONT }}>Where should we send your full results?</h2>
                <p style={{ fontSize: 14, color: C.gray600, marginBottom: 20, fontFamily: FONT, lineHeight: 1.55 }}>
                  We'll email you a copy of your personalised projection.
                </p>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                  ...
                  <Btn onClick={submitToGHL} disabled={!s2ok || submitting}>
                    {submitting ? "Preparing..." : "Unlock My Full Results"}
                  </Btn>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div>
                {/* Assumption callout — Change 1a */}
                <div style={{ textAlign: "center", marginBottom: 24, ...fadeIn(0) }}>
                  <div style={{ display: "inline-block", background: C.greenPale, borderRadius: 50, padding: "5px 16px", marginBottom: 12 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: C.green, fontFamily: FONT }}>{firstName}'s Debt Flip Projection</span>
                  </div>
                  <div style={{ background: C.white, border: `1.5px solid ${C.greenMist}`, borderRadius: 10, padding: 14, marginTop: 8 }}>
                    <p style={{ fontSize: 14, fontWeight: 600, color: C.green, fontFamily: FONT, margin: 0, lineHeight: 1.5 }}>
                      These numbers assume no extra money — just your current payments, redirected.
                    </p>
                  </div>
                </div>

                {/* HEADLINE: Debt-free date — Change 2 reorder */}
                <div style={fadeIn(0)}>
                  <div style={{ background: C.green, borderRadius: 12, padding: 28, textAlign: "center", marginBottom: 18, position: "relative", overflow: "hidden" }}>
                    ...
                    <div style={{ fontSize: 12, fontWeight: 700, color: C.greenPale, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8, fontFamily: FONT, position: "relative" }}>
                      You could be completely debt-free by
                    </div>
                    <div style={{ fontSize: 38, fontWeight: 800, color: C.white, fontFamily: FONT, lineHeight: 1.1, position: "relative" }}>
                      {flipDebtFreeDate}
                    </div>
                    <div style={{ fontSize: 16, fontWeight: 600, color: C.greenPale, fontFamily: FONT, marginTop: 8, position: "relative" }}>
                      That's {yrsSaved > 0 ? fmtYears(yrsSaved) : "—"} sooner than your current path
                    </div>
                  </div>
                </div>

                {/* SECOND: Time saved visual — comparison bars */}
                <div style={fadeIn(1)}>
                ...
                {/* THIRD: Interest saved + current situation */}
                <div style={fadeIn(2)}>
                  <div style={{ background: C.white, border: `1.5px solid ${C.gray100}`, borderRadius: 12, padding: 18, marginBottom: 18 }}>
                    <h3 style={{ fontSize: 13, fontWeight: 700, color: C.gray600, textTransform: "uppercase", letterSpacing: "0.06em", fontFamily: FONT, margin: "0 0 14px 0" }}>
                      The full picture
                    </h3>

                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 10 }}>
                      <StatBox label="Total Debt (all sources)" value={fmt(totalAllBal)} />
                      <StatBox label="You're Paying Monthly" value={fmt(totalAllPmt)} />
                    </div>
                    ...
                    <div style={{ background: C.greenPale, borderRadius: 10, padding: 16, textAlign: "center", border: `1px solid ${C.greenMist}` }}>
                      <div style={{ fontSize: 12, color: C.green, fontWeight: 600, marginBottom: 4, fontFamily: FONT }}>Your Debt Flip Repayment</div>
                      <div style={{ fontSize: 32, fontWeight: 800, color: C.green, fontFamily: FONT }}>
                        {fmt(Math.round(flipPmt))}<span style={{ fontSize: 15, fontWeight: 500, color: C.gray400 }}>/mth</span>
                      </div>
                      <div style={{ fontSize: 13, color: C.gray600, fontFamily: FONT, marginTop: 4, fontStyle: "italic" }}>
                ...
                <div style={fadeIn(3)}>
                  ...
                  <div style={{ background: C.green, borderRadius: 12, padding: 28, textAlign: "center", marginBottom: 18 }}>
                    <h3 style={{ fontSize: 20, fontWeight: 800, color: C.white, fontFamily: FONT, margin: "0 0 8px 0" }}>
                      Want to see if these numbers hold up?
                    </h3>
                    ...
                    <a
                      ...
                      style={{ display: "inline-block", padding: "16px 36px", background: C.white, color: C.green, borderRadius: 8, fontSize: 16, fontWeight: 700, fontFamily: FONT, textDecoration: "none", letterSpacing: "0.01em" }}>
                ...
                {/* Assumptions footnote — Change 1b */}
                <div style={fadeIn(4)}>
